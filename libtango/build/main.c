...
void create_kanjis_json_array(const entry* e, char* buffer) {
...
void create_readings_json_array(const entry* e, char* buffer) {
...
void create_senses_json_array(const entry* e, char* buffer) {
    if (e->senses_count == 0) {
        buffer[0] = '\0';
    ...
    for (int i = 0; i < e->senses_count; i++) {
        ...
        if (e->senses[i].examples[0].ex_srce[0] != '\0' || e->senses[i].examples[0].ex_text[0] != '\0' || e->senses[i].examples[0].ex_sent_count > 0) {
            ...
            if (e->senses[i].examples[0].ex_text[0] != '\0') {
                if (e->senses[i].examples[0].ex_srce[0] != '\0') strcat(buffer, ",");
                strcat(buffer, "\"ex_text\": \"");
                strcat(buffer, e->senses[i].examples[0].ex_text);
                strcat(buffer, "\"");
            }
            if (e->senses[i].examples[0].ex_sent_count > 0) {
                if (e->senses[i].examples[0].ex_srce[0] != '\0' || e->senses[i].examples[0].ex_text[0] != '\0') strcat(buffer, ",");
        ...
        if (cur_node->type == XML_ELEMENT_NODE)
        {
            if (xmlStrcmp(cur_node->name, (const xmlChar *)"entry") == 0)
            {
                ...
                CREATE TABLE entries (
                    ...
                    kanjis TEXT NOT NULL,    -- JSON almacenado como TEXT
                    readings TEXT NOT NULL,  -- JSON almacenado como TEXT
                    senses TEXT NOT NULL     -- JSON almacenado como TEXT
                );
                ...
                */
                sqlite3_stmt* stmt;
                const char* sql = "INSERT INTO entries (id, priority, kanjis, readings, senses) VALUES (?, ?, ?, ?, ?);";
                int ent_seq = e.ent_seq;
                int priority = e.priority;
                char kanjis[BUFFER_SIZE] = {0};
                char readings[BUFFER_SIZE] = {0};
                char senses[BUFFER_SIZE] = {0};
                // CREATE JSON strings for kanjis, readings, and senses
                create_kanjis_json_array(&e, kanjis);
                create_readings_json_array(&e, readings);
                create_senses_json_array(&e, senses);
                ...
                if (strlen(kanjis) == 0) {
                    sqlite3_bind_null(stmt, 3); // Bind NULL if kanjis is empty
                } else {
                    sqlite3_bind_text(stmt, 3, kanjis, -1, SQLITE_STATIC);
                }
                if (strlen(readings) == 0) {
                    sqlite3_bind_null(stmt, 4); // Bind NULL if readings is empty
                } else {
                    sqlite3_bind_text(stmt, 4, readings, -1, SQLITE_STATIC);
                }
                if (strlen(senses) == 0) {
                    sqlite3_bind_null(stmt, 5); // Bind NULL if senses is empty
                } else {
                    sqlite3_bind_text(stmt, 5, senses, -1, SQLITE_STATIC);
                }
