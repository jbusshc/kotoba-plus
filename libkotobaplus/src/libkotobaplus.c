...
KPDB* KP_db_open(const char* db_path) {
    KPDB* db = (KPDB*)malloc(sizeof(KPDB));
    if (!db) return NULL;

    memset(db, 0, sizeof(KPDB)); // inicializa todo a 0/NULL
...
static void prepare_critical_statements(KPDB* db) {
    if (!db || !db->db) return;
...
void parse_entry_json(const char* json, entry* e) {
    ...
    if (sense && cJSON_IsArray(sense)) {
        ...
        for (int i = 0; i < e->senses_count; i++) {
            ...
            // stagk
            cJSON* stagk = cJSON_GetObjectItem(sobj, "stagk");
            if (stagk && cJSON_IsArray(stagk)) {
                e->senses[i].stagk_count = cJSON_GetArraySize(stagk);
                for (int j = 0; j < e->senses[i].stagk_count; j++) {
                    cJSON* s = cJSON_GetArrayItem(stagk, j);
                    if (s && cJSON_IsString(s)) {
                        strncpy(e->senses[i].stagk[j], s->valuestring, sizeof(e->senses[i].stagk[j]) - 1);
                    }
                }
            }

            // stagr
            cJSON* stagr = cJSON_GetObjectItem(sobj, "stagr");
            if (stagr && cJSON_IsArray(stagr)) {
                e->senses[i].stagr_count = cJSON_GetArraySize(stagr);
                for (int j = 0; j < e->senses[i].stagr_count; j++) {
                    cJSON* s = cJSON_GetArrayItem(stagr, j);
                    if (s && cJSON_IsString(s)) {
                        strncpy(e->senses[i].stagr[j], s->valuestring, sizeof(e->senses[i].stagr[j]) - 1);
                    }
                }
            }
            ...
            // xref
            cJSON* xref = cJSON_GetObjectItem(sobj, "xref");
            if (xref && cJSON_IsArray(xref)) {
                e->senses[i].xref_count = cJSON_GetArraySize(xref);
                for (int j = 0; j < e->senses[i].xref_count; j++) {
                    cJSON* s = cJSON_GetArrayItem(xref, j);
                    if (s && cJSON_IsString(s)) {
                        strncpy(e->senses[i].xref[j], s->valuestring, sizeof(e->senses[i].xref[j]) - 1);
                    }
                }
            }

            // ant
            cJSON* ant = cJSON_GetObjectItem(sobj, "ant");
            if (ant && cJSON_IsArray(ant)) {
                e->senses[i].ant_count = cJSON_GetArraySize(ant);
                for (int j = 0; j < e->senses[i].ant_count; j++) {
                    cJSON* s = cJSON_GetArrayItem(ant, j);
                    if (s && cJSON_IsString(s)) {
                        strncpy(e->senses[i].ant[j], s->valuestring, sizeof(e->senses[i].ant[j]) - 1);
                    }
                }
            }

            // field
            cJSON* field = cJSON_GetObjectItem(sobj, "field");
            if (field && cJSON_IsArray(field)) {
                e->senses[i].field_count = cJSON_GetArraySize(field);
                for (int j = 0; j < e->senses[i].field_count; j++) {
                    cJSON* s = cJSON_GetArrayItem(field, j);
                    if (s && cJSON_IsString(s)) {
                        strncpy(e->senses[i].field[j], s->valuestring, sizeof(e->senses[i].field[j]) - 1);
                    }
                }
            }
            ...
            if (misc && cJSON_IsArray(misc)) {
                ...
                for (int j = 0; j < e->senses[i].misc_count; j++) {
                    cJSON* s = cJSON_GetArrayItem(misc, j);
                    if (s && cJSON_IsString(s)) {
                        strncpy(e->senses[i].misc[j], s->valuestring, sizeof(e->senses[i].misc[j]) - 1);
                    }
                }
            }

            // s_inf
            cJSON* s_inf = cJSON_GetObjectItem(sobj, "s_inf");
            if (s_inf && cJSON_IsArray(s_inf)) {
                e->senses[i].s_inf_count = cJSON_GetArraySize(s_inf);
                for (int j = 0; j < e->senses[i].s_inf_count; j++) {
                    cJSON* s = cJSON_GetArrayItem(s_inf, j);
                    if (s && cJSON_IsString(s)) {
                        strncpy(e->senses[i].s_inf[j], s->valuestring, sizeof(e->senses[i].s_inf[j]) - 1);
                    }
                }
            }

            // lsource
            cJSON* lsource = cJSON_GetObjectItem(sobj, "lsource");
            if (lsource && cJSON_IsArray(lsource)) {
                e->senses[i].lsource_count = cJSON_GetArraySize(lsource);
                for (int j = 0; j < e->senses[i].lsource_count; j++) {
                    cJSON* s = cJSON_GetArrayItem(lsource, j);
                    if (s && cJSON_IsString(s)) {
                        strncpy(e->senses[i].lsource[j], s->valuestring, sizeof(e->senses[i].lsource[j]) - 1);
                    }
                }
            }

            // dial
            cJSON* dial = cJSON_GetObjectItem(sobj, "dial");
            if (dial && cJSON_IsArray(dial)) {
                e->senses[i].dial_count = cJSON_GetArraySize(dial);
                for (int j = 0; j < e->senses[i].dial_count; j++) {
                    cJSON* s = cJSON_GetArrayItem(dial, j);
                    if (s && cJSON_IsString(s)) {
                        strncpy(e->senses[i].dial[j], s->valuestring, sizeof(e->senses[i].dial[j]) - 1);
                    }
                }
            }
...
void KP_db_text_search(KPDB* db, const char* termino, KPSearchResultCallback callback, void* userdata) {
...
void KP_db_id_search(KPDB* db, int id, entry* e, KPEntryCallback callback, void* userdata) {
...
void KP_update_srs_review(KPDB* Tdb, int entry_id, int quality) {
    ...
    KP_update_srs_stats(Tdb);
}

int KP_get_total_cards(KPDB* db) {
    if (!db) return 0;
...
int KP_get_due_review_count(KPDB* db) {
    if (!db) return 0;
...
void KP_update_srs_stats(KPDB* db) {
    if (!db) return;

    // Actualizar estadísticas
    db->stats.total_cards = KP_get_total_cards(db);
    db->stats.due_review_count = KP_get_due_review_count(db);
    db->stats.last_update = time(NULL);
}

void KP_review_card_buffer_init(KPDB* db, review_card_buffer* buffer) {
    if (!db || !buffer) return;

    KP_update_srs_stats(db);
    
    // Limpiar el buffer
    KP_review_card_buffer_clear(buffer);

    if(db->stats.due_review_count > 0) {
        // Llenar el buffer con las cartas pendientes de revisión
        KP_review_card_buffer_fill(db, buffer, BUFFER_LIMIT);
    }
}

void KP_review_card_buffer_fill(KPDB* db, review_card_buffer* buffer, int cards_to_add) {
...
review_card* KP_review_card_buffer_get_current(review_card_buffer* buffer) {
    if (!buffer || buffer->count == 0) return NULL;

    return &buffer->cards[buffer->current_index];
}

void KP_review_card_buffer_advance(review_card_buffer* buffer) {
    if (!buffer || buffer->count == 0) return;

    buffer->current_index = (buffer->current_index + 1) % buffer->count;
}

void KP_review_card_buffer_undo(review_card_buffer* buffer) {
    if (!buffer || buffer->count == 0) return;

    buffer->current_index = (buffer->current_index + buffer->count - 1) % buffer->count;
}

void KP_review_card_buffer_clear(review_card_buffer* buffer) {
    if (!buffer) return;
    buffer->count = 0;
...
int KP_get_pending_review_cards(KPDB* db) {
    if (!db) return 0;

    // Reuse KP_get_due_review_count (already uses prepared stmt if available)
    return KP_get_due_review_count(db);
}

srs_stats* KP_get_srs_stats(KPDB* db) {
    if (!db) return NULL;
...
int KP_srs_load(KPDB* db, int entry_id, srs_review* out) {
    if (!db || !db->db || !out) return 0;

    sqlite3_stmt* stmt = NULL;

    // Intentar usar prepared statement
    if (db->stmts.stmt_srs_select) {
        stmt = db->stmts.stmt_srs_select;
        sqlite3_reset(stmt);
        sqlite3_clear_bindings(stmt);
        sqlite3_bind_int(stmt, 1, entry_id);
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            out->last_review  = sqlite3_column_int64(stmt, 0);
            out->interval     = sqlite3_column_int(stmt, 1);
            out->ease_factor  = sqlite3_column_double(stmt, 2);
            out->repetitions  = sqlite3_column_int(stmt, 3);
            out->due_date     = sqlite3_column_int64(stmt, 4);
    ...
    // Fallback
    const char* sql =
        "SELECT last_review, interval, ease_factor, repetitions, due_date "
        "FROM srs_reviews WHERE entry_id = ?;";
    
    if (sqlite3_prepare_v2(db->db, sql, -1, &stmt, NULL) != SQLITE_OK) {
        fprintf(stderr, "❌ Error SELECT SRS: %s\n", sqlite3_errmsg(db->db));
        return 0;
    }
